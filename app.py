...
def processar_nota():
    ...
    try:
        ...
        if not isinstance(itens, list):
            flash("A IA retornou formato inválido.", "danger")
            return redirect("/compras-inteligentes")

        return render_template("resultado_nota.html", itens=itens)

    except json.JSONDecodeError:
        flash("A IA retornou um JSON inválido.", "danger")
        return redirect("/compras-inteligentes")
    except Exception as e:
        log_erro(f"Erro ao processar nota: {e}")
        flash("Erro ao processar a nota fiscal.", "danger")
        return redirect("/compras-inteligentes")
    finally:
...
def confirmar_nota():
    try:
        ...
        with get_conn() as conn:
            with conn.cursor() as cur:
                for i in range(total):
                    ...
                    cur.execute(
                        "SELECT id_materia_prima FROM materia_prima WHERE LOWER(nome) = LOWER(%s)", (nome,)
                    )
                    ...
                    if materia:
                        ...
                        cur.execute(
                            "INSERT INTO materia_prima (nome, unidade_medida, preco_unitario) "
                            "VALUES (%s,'UN',%s) RETURNING id_materia_prima",
                            (nome, preco),
                        )
                    ...
                    cur.execute(
                        ...
                        "VALUES (%s,'entrada',%s,'Importação via IA',%s)",
        ...
        log_erro(f"Erro ao confirmar nota: {e}")
